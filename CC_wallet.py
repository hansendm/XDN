...
word_counts = {}
# file_type_catalog[file_type_extension]=[filepath]
file_type_catalog = {}
...
image_file_types = [".jpg", ".jpeg", ".jfif", ".pjpeg", ".pjp", ".png", ".gif", ".apng", ".avif", ".svg", ".webp",
...
def new_project():
    ...
    define_directories()
    modify_keywords()
    wordsearch_directory(directory, keywords)
...
def wordsearch_dict_filename_lists(words):
    # create nested dictionary
    for k in file_type_catalog.keys():
        # go thru imagery filename dictionary to check for matching
        if k in image_file_types:
            matched_dict = match_filenames(file_type_catalog[k], words)
            if len(matched_dict) > 0:
                word_matched_image_files[k] = matched_dict
            else:
                other_image_files[k] = matched_dict
        # if not image file, catalog other files
        else:
            matched_dict = match_filenames(file_type_catalog[k], words)
            if len(matched_dict) > 0:
...
def wordsearch_file(filename, words):
    with open(filename) as df:
        data = df.read()
        sp = data.split()
        for x in words:
            ...
            for nw in li:
                if nw in word_matched_files.keys():
                    if filename not in word_matched_files[nw]:
                        word_matched_files[nw].append(filename)
    return
...
def catalog_files(directory):
    ...
    for root, dirs, files in os.walk(directory):
        for filename in files:
            ...
            if extension not in file_type_catalog.keys():
                file_type_catalog[extension] = []
            file_type_catalog[extension].append(filepath)
    print("Files cataloged:")
    for k in file_type_catalog.keys():
        print('    %-7s %d' % (k, len(file_type_catalog[k])))
    return
...
#	get_dic_from_saved_files(file_type_catalog,file_type_list_dir)
...
def search_files_word_matches():
    for k in file_type_catalog.keys():
        ...
        if k not in image_file_types:
            for f in file_type_catalog[k]:
                wordsearch_file(filename, list_of_words)
    return


def wordsearch_directory(directory, list_of_words):
    catalog_files(directory)
    save_dict_values_by_keys(file_type_catalog, file_type_list_dir)
    print("File type lists are saved")
    print("Filename Word Search in Progress..")
    wordsearch_dict_filename_lists(list_of_words)
